...
exports.getDemandas = async (req, res) => {
  try {
    ...
    if (req.usuario.role === "admin") {
      ...
      // As demandas recusadas agora são enviadas para contabilização nas estatísticas
    }
...
exports.createDemanda = async (req, res) => {
  try {
    const { titulo, descricao, dataEsperada, solicitante } = req.body
    ...
    const demanda = await Demanda.create({
      ...
      adminId: idDaEmpresa,
      status: "Em análise",
      solicitante: req.usuario.role === "admin" ? solicitante : null,
...
exports.getComentariosPrivados = async (req, res) => {
  try {
    const { id } = req.params
    ...
    const isCreator = req.usuario.id === demanda.userId
    const belongsToAdmin = req.usuario.id === demanda.adminId

    if (!isCreator && !(isAdmin && belongsToAdmin)) {
      return res.status(403).json({ erro: "Acesso negado aos comentários desta demanda" })
    }

    const comentarios = await ComentarioPrivado.findAll({
      where: { demandaId: id },
      include: [{ model: User, as: "User", attributes: ["id", "nome", "role"] }],
      order: [["createdAt", "ASC"]]
    })
    ...
    console.error("Erro no getComentariosPrivados:", error)
    res.status(500).json({ erro: "Erro ao buscar comentários" })
  }
}


exports.addComentarioPrivado = async (req, res) => {
  try {
    const { id } = req.params
    ...
    if (demanda.status === "Recusada") {
      return res.status(403).json({ erro: "Não é possível comentar em uma demanda recusada" })
    }
    ...
    if (!isCreator && !(isAdmin && belongsToAdmin)) {
      return res.status(403).json({ erro: "Acesso negado para comentar nesta demanda" })
    }

    const comentario = await ComentarioPrivado.create({
      texto,
      demandaId: id,
      userId: req.usuario.id
    })

    const comentarioComUser = await ComentarioPrivado.findByPk(comentario.id, {
      include: [{ model: User, as: "User", attributes: ["id", "nome", "role"] }]
    })

    res.status(201).json(comentarioComUser)

  } catch (error) {
    console.error("Erro no addComentarioPrivado:", error)
