...
describe('DemandaForm.vue', () => {
  ...
  // =====================================================
  // COBERTURA EXTRA ADMIN
  // =====================================================

  test('deve exibir e preencher os campos de admin na edição', async () => {
    api.put.mockResolvedValue({})
    const demanda = { id: 1, titulo: 'T', status: 'Em análise', descricao: 'D', dataEsperada: '' }
    const wrapper = mount(DemandaForm, { props: { demanda, isStudent: false } })
    
    // Altera o status
    const select = wrapper.find('select')
    await select.setValue('Concluída')
    
    // Adiciona uma mensagem de atualização (campo só aparece na edição para admin)
    const inputs = wrapper.findAll('input')
    // O campo de titulo é o primeiro, a nota de atualização é o último input texto
    const msgInput = inputs[inputs.length - 1]
    await msgInput.setValue('Mensagem de teste')
    
    await wrapper.find('form').trigger('submit.prevent')
    
    expect(api.put).toHaveBeenCalledWith('/demandas/1', expect.objectContaining({
      status: 'Concluída',
