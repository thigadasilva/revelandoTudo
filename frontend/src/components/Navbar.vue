<template>
  <nav class="navbar glass-navbar">
    <div class="nav-left">
      ...
      <template v-if="role === 'student' && user.Turma">
        <span class="divider hide-sm"></span>
        <span class="turma-indicator hide-sm">
          Turma: {{ user.Turma.nome }} | Código: <strong>{{ user.Turma.codigo }}</strong>
        </span>
      </template>
    </div>
